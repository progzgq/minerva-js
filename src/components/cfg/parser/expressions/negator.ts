...
function negateTruthiness(expression: ESTree.Expression): ESTree.Expression {
  if (expression.type === ESTree.NodeType.Literal) {
    ...
    if (ESTree.isNullLiteral(literal)) {
...
function invertEqualityComparisonOperator(
  ...
  return ESTree.binaryExpression(
    //@ts-ignore
    firstCharOfInvertedOperator + restOfInvertedOperator,
...
function invertLogicalExpression(
  ...
  return ESTree.logicalExpression(
    invertedOperator,
    negateTruthiness(logicalExpression.left),
    negateTruthiness(logicalExpression.right)
  );
  // return {
