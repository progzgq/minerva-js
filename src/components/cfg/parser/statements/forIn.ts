...
function parseForInStatement(
  ...
  let conditionNode = context
    ...
    let isDoneExpression: ESTree.MemberExpression = ESTree.memberExpression(
      iteratorIdentifier,
  ...
  let nextElementCallee: ESTree.MemberExpression = ESTree.memberExpression(
    iteratorIdentifier,
    createIdentifier("next"),
    false
  )  
...
function getLeftHandSideOfAssignment(
  ...
  if (forInStatement.left.type === ESTree.NodeType.VariableDeclaration) {
    let variableDeclarator:VariableDeclarator = (<ESTree.VariableDeclaration>forInStatement.left)
      .declarations[0];
