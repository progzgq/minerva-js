...
function isCompileTimeConstant(expression: ESTree.Expression|ESTree.Statement|ESTree.VariableDeclarator): boolean {
...
function isAlwaysTruthy(expression: ESTree.Expression|ESTree.Statement|ESTree.VariableDeclarator): boolean {
  switch (expression.type) {
    case ESTree.NodeType.Literal:
      return !types.isNullLiteral(expression);// !!(<ESTree.Literal>expression).value;
